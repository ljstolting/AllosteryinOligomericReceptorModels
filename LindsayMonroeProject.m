...
if length(resp_meas)>=length(char(edges(2)))
    ...
    if ismember('custom',temp(1))==true
       if length(temp)==1
            disp('Order:')
            disp(char(C+'a'-1))
    ...
    if ismember('custom',temp)==false
        temp2 = length(char(edges(2))):length(char(edges(2))):length(resp_meas);
        temp2 = [0 temp2];
        indecies = [];
        for i=1:length(temp2)-1
            [~, idx] = ismember(resp_meas(temp2(i)+1:temp2(i+1)),monnames);
...
x = logspace(-3,3,1000);
for i = 1:2^k
    plot(x,pi_fun(dimerfits_SSQs(i,1:length(parnames)),x))
end

if Simulate(1) == true
    labels = ["Simulated Data", "Template Function"];
end
if Simulate(1) == false
    labels = ["Sample Data"];
end
...
if length(parnames)<4
    output = table(modeltable, AICs,parvals);
    output.Properties.VariableNames = {'Model','AIC','Parameter Values'};
    disp(output)
end
if length(parnames)>3
    output = table(modeltable, AICs);
    output.Properties.VariableNames = {'Model','AIC'};
    disp(output)
    disp(parvals)
end
